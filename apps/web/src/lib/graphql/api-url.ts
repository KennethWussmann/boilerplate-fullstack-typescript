...
export const getWsApiUrl = () => {
    const url = getSettingsValue(`${settingsLocalStorageBaseKey}.backend.wsUrl`, wsApiUrl ?? deriveWebSocketUrl(getHttpApiUrl()))
    return ensureGraphqlPath(url)
}
export const isApiEnabled = () => {
    const enabled = getSettingsValue(`${settingsLocalStorageBaseKey}.backend.enabled`, String(import.meta.env.VITE_API_ENABLED !== 'false'))
